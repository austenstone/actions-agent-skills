...
2. **Recover from hard-failed online audits.** If zizmor prints `fatal: no audit was performed`, emits empty stdout, or fails before valid JSON, do not report clean. For a local scan, rerun with `--no-online-audits`, then `--offline`. For a remote slug, rerun with `--no-online-audits` only — `--offline` cannot fetch the repository at all, and zizmor's own help says to remove it. If a remote scan reports `no inputs collected`, no flag recovers it: clone the repository and scan `.github/workflows/` locally (verified on `pandas-dev/pandas`, which collected 0 inputs remotely but returned 21 findings locally). Label successful fallback reports as skipping network-dependent audits: `impostor-commit`, `ref-confusion`, `known-vulnerable-actions`, and `stale-action-refs`.
